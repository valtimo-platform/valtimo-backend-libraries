...
internal class LockedDocumentJsonValueResolverTest {
    ...
    private lateinit var documentValueResolver: LockedDocumentJsonValueResolverFactory

    private lateinit var processInstanceId: String
    private lateinit var variableScope: DelegateTaskFake
    private lateinit var documentInstanceId: Document.Id
    ...
    internal fun setUp() {
        ...
        documentValueResolver = LockedDocumentJsonValueResolverFactory(
        ...
        documentInstanceId = mock<Document.Id>()
        document = mock()
        whenever(processDocumentService.getDocumentId(CamundaProcessInstanceId(processInstanceId), variableScope)).thenReturn(documentInstanceId)
        whenever(documentService.findByWithLock(documentInstanceId)).thenReturn(Optional.of(document))
    }
    ...
    fun `should add text value`() {
        ...
        verify(documentService).modifyDocumentWithLock(eq(document), captor.capture())
    ...
    fun `should get property names from referenced nested object`() {
        ...
        Assertions.assertArrayEquals(
            paths.toTypedArray(), arrayOf(
                "doc-locked:/address/city",
                "doc-locked:/address/country",
                "doc-locked:/address/number",
                "doc-locked:/address/province",
                "doc-locked:/address/streetName"
            )
        )
    }
    ...
    fun `should get array property with nested properties`() {
        ...
        assertEquals("doc-locked:/object1/object2/array1", options[0].path)
