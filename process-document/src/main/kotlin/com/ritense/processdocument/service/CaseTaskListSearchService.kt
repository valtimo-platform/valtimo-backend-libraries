...
class CaseTaskListSearchService(
    ...
    private fun constructWhere(
        ...
        val authorizationPredicate: Predicate =
            getAuthorizationSpecification(OperatonTaskActionProvider.VIEW_LIST).toPredicate(taskRoot, query, cb)
    ...
    private fun handleHideInaccessibleTasksFilter(
        ...
        return if (values.size == 1 && values[0] == true) {
            getAuthorizationSpecification(OperatonTaskActionProvider.VIEW).toPredicate(taskRoot, query, cb)
        } else {
