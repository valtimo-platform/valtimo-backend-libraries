...
import static com.ritense.search.domain.FieldType.MULTI_SELECT_DROPDOWN;
import static com.ritense.search.domain.FieldType.RANGE;
import static com.ritense.search.domain.SearchFieldMatchType.LIKE;
import static com.ritense.search.domain.SearchFieldMatchType.EXACT;
...
public class SearchRequestMapper {
    ...
    private static DatabaseSearchType findDatabaseSearchType(SearchWithConfigRequest.SearchWithConfigFilter searchFilter, SearchFieldV2 searchField) {
    ...
    private static LocalDate findSingleDateSearchFieldLocalDate(SearchFieldV2 searchField, List<SearchRequestValue> searchRequestValues) {
        ...
        if (firstEntry.isPresent()
            ...
            && searchField.getDataType() == DataType.DATE
    ...
    private static Boolean isRangeDateSearchField(SearchFieldV2 searchField, SearchRequestValue rangeFrom, SearchRequestValue rangeTo) {
        return rangeFrom.getComparableValue() instanceof LocalDate
            && rangeTo.getComparableValue() instanceof LocalDate
            && searchField.getDataType() == DataType.DATE
            && searchField.getFieldType() == FieldType.RANGE;
    }

    private static AdvancedSearchRequest.OtherFilter singleDateToDateTimeRangeFilter(SearchFieldV2 searchField, LocalDate localDateValue, ZoneOffset zoneOffset) {
