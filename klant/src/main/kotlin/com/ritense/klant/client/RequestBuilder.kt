...
class RequestBuilder {
    ...
    companion object {
        fun builder() = Builder(restTemplate = RestTemplateBuilderSingleton.get().build())
    }
    ...
    data class Builder(
        ...
        fun <T> executePaged(responseClass: Class<out T>): ResultPage<T> {
            try {
                prepare()
                val responseEntity = restTemplate.exchange(
                    url!!,
        ...
        private fun <T> getTypeWrapped(responseClass: Class<out T>): ParameterizedTypeReference<ResultPage<T>> {
            val type: ParameterizedTypeReference<ResultPage<T>> = ParameterizedTypeReference.forType(
                ResolvableType.forClassWithGenerics(ResultPage::class.java, responseClass).type
            )
            return type
        }
