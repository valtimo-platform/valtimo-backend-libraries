...
open class TemplateDeploymentService(
    private val resourceLoader: ResourceLoader,
    private val templateService: TemplateService,
) {

    @EventListener(ApplicationReadyEvent::class)
    open fun deployTemplates() {
            ...
            loadResources().forEach { resource ->
                ...
                resource.inputStream.bufferedReader().use { reader ->
                    templateService.saveTemplate(
                        ValtimoTemplate(
                            key = fileName.substringAfterLast('/').substringBeforeLast(".template.").lowercase().replace("[^a-z0-9_]+".toRegex(), "-"),
    ...
    companion object {
        ...
        const val PATH = "classpath*:**/template/*.template.*"
    }
}