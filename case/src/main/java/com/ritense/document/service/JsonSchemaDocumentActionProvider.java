...
public class JsonSchemaDocumentActionProvider implements ResourceActionProvider<JsonSchemaDocument> {
    ...
    public static final Action<JsonSchemaDocument> EXPORT_LIST = new Action<>(Action.EXPORT_LIST);

    @NotNull
    @Override
    public List<Action<JsonSchemaDocument>> getAvailableActions() {
        return List.of(VIEW, VIEW_LIST, CREATE, MODIFY, DELETE, CLAIM, ASSIGN, ASSIGNABLE, EXPORT_LIST);
    }
}
