...
class WordpressMailDispatcher(
    ...
    override fun send(rawMailMessage: RawMailMessage): List<MailMessageStatus> {
        return getWordpressMailConnector().send(rawMailMessage)
    }

    override fun send(templatedMailMessage: TemplatedMailMessage): List<MailMessageStatus> {
        return getWordpressMailConnector().send(templatedMailMessage)
    }

    override fun getMaximumSizeAttachments(): Int {
        return getWordpressMailConnector().getMaximumSizeAttachments()
    }

    private fun getWordpressMailConnector(): WordpressMailConnector {
        return connectorService.loadByClassName(WordpressMailConnector::class.java)
    }
