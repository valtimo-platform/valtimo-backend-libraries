...
public class RedirectToFilter implements MailFilter {

    private final MailingProperties mailingProperties;

    @Override
    public Optional<RawMailMessage> doFilter(RawMailMessage rawMailMessage) {
        rawMailMessage.recipients.filterBy(recipient -> mailingProperties.getSendRedirectedMailsTo().contains(recipient));
        return Optional.of(rawMailMessage);
    }

    @Override
    public Optional<TemplatedMailMessage> doFilter(TemplatedMailMessage templatedMailMessage) {
        templatedMailMessage.recipients.filterBy(recipient -> mailingProperties.getSendRedirectedMailsTo().contains(recipient));
    ...
    public int getPriority() {
        return mailingProperties.getRedirectAllMailsPriority();
    }
}