...
class OperatonTaskServiceTest {
    ...
    private OperatonTaskRepository operatonTaskRepository;
    ...
    void setUp() {
        ...
        operatonTaskRepository = mock(OperatonTaskRepository.class);
    ...
    void findTaskById_taskDoesNotExists() {
        assertThrows(TaskNotFoundException.class, () -> operatonTaskService.findTaskById(TASK_ID));
    }
    ...
    void assignTask_taskDoesNotExist() {
        ...
        assertThrows(TaskNotFoundException.class, () -> operatonTaskService.assign(TASK_ID, ASSIGNEE));
    }
    ...
    void assignTask_taskExists() {
        ...
        when(operatonTaskRepository.findById(any()))
        ...
        try (MockedStatic<SecurityUtils> securityUtils = mockStatic(SecurityUtils.class)) {
            ...
            operatonTaskService.assign(TASK_ID, ASSIGNEE);
        }
        ...
        assertThat(event.getResultType()).isEqualTo("com.ritense.valtimo.operaton.domain.OperatonTask");
    ...
    void assignTask_taskExistsWithoutNewAssignee() {
        ...
        try (MockedStatic<SecurityUtils> securityUtils = mockStatic(SecurityUtils.class)) {
            ...
            operatonTaskService.assign(TASK_ID, null);
        }
    ...
    void assignTask_taskClaimedDoesntExist() {
        ...
        assertThrows(IllegalStateException.class, () -> operatonTaskService.assign(TASK_ID, ASSIGNEE));
    ...
    void unassignTask_taskDoesNotExist() {
        doThrow(new IllegalStateException()).when(operatonTaskService).findTaskById(TASK_ID);
        assertThrows(IllegalStateException.class, () -> operatonTaskService.unassign(TASK_ID));
        verify(outboxService, times(0)).send(any());
    }
    ...
    void unassignTask_taskExists() {
        ...
        operatonTaskService.unassign(TASK_ID);
    ...
    void taskComplete_withVariablesAndValidationException() {
        ...
        assertThrows(
            ...
            () -> operatonTaskService.completeTaskWithFormData(TASK_ID, variables)
        );
        verify(outboxService, times(0)).send(any());
    }
    ...
    void taskComplete_withoutVariablesSuccessfully() {
        ...
        OperatonTaskService operatonTaskService = spy(new OperatonTaskService(
            taskService,
            null,
            delegateTaskHelper,
            operatonTaskRepository,
        ...
        operatonTaskService.completeTaskWithFormData(TASK_ID, null);
        verify(operatonTaskService, times(1)).completeTaskWithFormData(TASK_ID, null);
