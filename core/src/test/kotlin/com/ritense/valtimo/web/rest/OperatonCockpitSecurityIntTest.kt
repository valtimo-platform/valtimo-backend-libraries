...
@Tag("security")
class OperatonCockpitSecurityIntTest {
    ...
    inner class LocalhostWhiteListed @Autowired constructor(
        ...
        @Test
        fun `localhost should be whitelisted to access operaton cockpit endpoints`() {
            val restTemplate = RestTemplate()
            val response = restTemplate.getForEntity("http://127.0.0.1:$serverPort/operaton/api/engine/engine/", String::class.java)
    ...
    inner class GoogleWhiteListed @Autowired constructor(
        ...
        @Test
        fun `localhost should not be whitelisted to access operaton cockpit endpoints`() {
            val restTemplate = RestTemplate()
            val forbidden = assertThrows<Forbidden> {
                restTemplate.getForEntity("http://127.0.0.1:$serverPort/operaton/api/engine/engine/", String::class.java)
            }
