...
class OperatonExecutionRepositoryIntTest @Autowired constructor(
    private val operatonExecutionRepository: OperatonExecutionRepository
): BaseIntegrationTest() {

    @Test
    @Transactional
    fun `should find a operaton execution`() {
        val instance = runtimeService.startProcessInstanceByKey(
            "one-task-process",
            UUID.randomUUID().toString()
        )

        val result = operatonExecutionRepository.findById(instance.id)
    ...
    @Test
    @Transactional
    fun `should find operaton execution variables`() {
