...
class CamundaByteArraySpecificationHelper {
    ...
    companion object {
        ...
        @JvmStatic
        fun byId(id: String) = Specification<CamundaBytearray> { root, _, cb ->
            cb.equal(root.get<Any>(ID), id)
        }

        @JvmStatic
        fun byRevision(revision: Int) = Specification<CamundaBytearray> { root, _, cb ->
            cb.equal(root.get<Any>(REVISION), revision)
        }

        @JvmStatic
        fun byName(name: String) = Specification<CamundaBytearray> { root, _, cb ->
            cb.equal(root.get<Any>(NAME), name)
        }

        @JvmStatic
        fun byDeploymentId(deploymentId: String) = Specification<CamundaBytearray> { root, _, cb ->
            cb.equal(root.get<Any>(DEPLOYMENT_ID), deploymentId)
        }

        @JvmStatic
        fun byBytes(bytes: ByteArray) = Specification<CamundaBytearray> { root, _, cb ->
            cb.equal(root.get<Any>(BYTES), bytes)
        }

        @JvmStatic
        fun byTenantId(tenantId: String) = Specification<CamundaBytearray> { root, _, cb ->
            cb.equal(root.get<Any>(TENANT_ID), tenantId)
        }

        @JvmStatic
        fun byType(type: Int) = Specification<CamundaBytearray> { root, _, cb ->
            cb.equal(root.get<Any>(TYPE), type)
        }

        @JvmStatic
        fun byCreateTime(createTime: LocalDateTime) = Specification<CamundaBytearray> { root, _, cb ->
            cb.equal(root.get<Any>(CREATE_TIME), createTime)
        }

        @JvmStatic
        fun byRootProcessInstanceId(rootProcessInstanceId: String) = Specification<CamundaBytearray> { root, _, cb ->
            cb.equal(root.get<Any>(ROOT_PROCESS_INSTANCE_ID), rootProcessInstanceId)
        }

        @JvmStatic
        fun byRemovalTime(removalTime: LocalDateTime) = Specification<CamundaBytearray> { root, _, cb ->
