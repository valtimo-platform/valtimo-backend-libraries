...
class OperatonTaskSpecificationHelper {
    ...
    companion object {
        ...
        @JvmStatic
        fun all() = Specification<OperatonTask> { _, _, cb ->
            cb.equal(cb.literal(1), 1)
        }

        @JvmStatic
        fun byId(taskId: String) = Specification<OperatonTask> { root, _, cb ->
            cb.equal(root.get<Any>(ID), taskId)
        }

        @JvmStatic
        fun byProcessInstanceId(processInstanceId: String) = Specification<OperatonTask> { root, _, cb ->
        ...
        fun byProcessDefinitionKeys(processDefinitionKeys: Collection<String>) =
            Specification<OperatonTask> { root, _, _ ->
                root.get<Any>(PROCESS_DEFINITION).get<Any>(KEY).`in`(processDefinitionKeys)
            }

        @JvmStatic
        fun byProcessDefinitionId(processDefinitionId: String) = Specification<OperatonTask> { root, _, cb ->
            cb.equal(root.get<Any>(PROCESS_DEFINITION).get<Any>(PROCESS_DEFINITION_ID), processDefinitionId)
        }

        @JvmStatic
        fun byCandidateGroups(candidateGroups: Collection<String>) = Specification<OperatonTask> { root, _, cb ->
        ...
        @JvmStatic
        fun byName(taskName: String) = Specification<OperatonTask> { root, _, cb ->
        ...
        @JvmStatic
        fun byUnassigned() = Specification<OperatonTask> { root, _, _ ->
            root.get<Any>(ASSIGNEE).isNull
        }

        @JvmStatic
        fun byAssigned() = Specification<OperatonTask> { root, _, _ ->
            root.get<Any>(ASSIGNEE).isNotNull
        }

        @JvmStatic
        fun byCreateTimeAfter(fromDate: LocalDateTime) = Specification<OperatonTask> { root, _, cb ->
            cb.greaterThan(root.get<Any>(CREATE_TIME).`as`(LocalDateTime::class.java), cb.literal(fromDate))
        }

        @JvmStatic
        fun byCreateTimeBefore(toDate: LocalDateTime) = Specification<OperatonTask> { root, _, cb ->
            cb.lessThan(root.get<Any>(CREATE_TIME).`as`(LocalDateTime::class.java), cb.literal(toDate))
        }

        @JvmStatic
        fun bySuspensionState(suspensionState: Int) = Specification<OperatonTask> { root, _, cb ->
