...
class OperatonVariableInstanceSpecificationHelper {
    ...
    companion object {
        ...
        @JvmStatic
        fun byName(name: String) = Specification<OperatonVariableInstance> { root, _, cb ->
            cb.equal(root.get<Any>(NAME), name)
        }

        @JvmStatic
        fun byNameIn(vararg names: String) = Specification<OperatonVariableInstance> { root, _, cb ->
            if (names.isEmpty()) {
                cb.equal(cb.literal(0), 1)
            } else {
                root.get<Any>(NAME).`in`(*names)
            }
        }

        @JvmStatic
        fun byProcessInstanceId(processInstanceId: String) = Specification<OperatonVariableInstance> { root, _, cb ->
            cb.equal(root.get<Any>(PROCESS_INSTANCE).get<Any>(ID), processInstanceId)
        }

        @JvmStatic
        fun byProcessInstanceIdIn(vararg processInstanceIds: String) =
            Specification<OperatonVariableInstance> { root, _, _ ->
                root.get<Any>(PROCESS_INSTANCE).get<Any>(ID).`in`(*processInstanceIds)
            }
