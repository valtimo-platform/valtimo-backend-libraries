...
class OperatonHistoricProcessInstanceSpecificationHelper {
    ...
    companion object {
        ...
        @JvmStatic
        fun query() = Specification<OperatonHistoricProcessInstance> { _, _, cb ->
            cb.equal(cb.literal(1), 1)
        }

        @JvmStatic
        fun byId(id: String) = Specification<OperatonHistoricProcessInstance> { root, _, cb ->
            cb.equal(root.get<Any>(ID), id)
        }

        @JvmStatic
        fun byProcessInstanceId(processInstanceId: String) =
            Specification<OperatonHistoricProcessInstance> { root, _, cb ->
        ...
        @JvmStatic
        fun byUnfinished() = Specification<OperatonHistoricProcessInstance> { root, _, _ ->
            root.get<Any>(END_TIME).isNull
        }

        @JvmStatic
        fun byFinished() = Specification<OperatonHistoricProcessInstance> { root, _, _ ->
            root.get<Any>(END_TIME).isNotNull
        }

        @JvmStatic
        fun byEndTimeAfter(dateTime: LocalDateTime) = Specification<OperatonHistoricProcessInstance> { root, _, cb ->
            cb.greaterThanOrEqualTo(root.get<Any>(END_TIME).`as`(LocalDateTime::class.java), dateTime)
        }

        @JvmStatic
        fun byEndTimeBefore(dateTime: LocalDateTime) = Specification<OperatonHistoricProcessInstance> { root, _, cb ->
            cb.lessThanOrEqualTo(root.get<Any>(END_TIME).`as`(LocalDateTime::class.java), dateTime)
        }

        @JvmStatic
        fun byStartUserId(startUserId: String) = Specification<OperatonHistoricProcessInstance> { root, _, cb ->
