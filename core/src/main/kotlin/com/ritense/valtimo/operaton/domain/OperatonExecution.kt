...
class OperatonExecution(
    ...
    val rootProcessInstance: OperatonExecution?,

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "PROC_INST_ID_", insertable = false, updatable = false)
    var processInstance: OperatonExecution?,
    ...
    @Transient
    override fun getVariableInstancesLocal(): Collection<OperatonVariableInstance> = variableInstances

    @Transient
    override fun getParentVariableScope(): OperatonVariableScope? = parent
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (other !is OperatonExecution) return false
