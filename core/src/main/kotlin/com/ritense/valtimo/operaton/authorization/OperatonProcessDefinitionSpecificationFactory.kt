...
class OperatonProcessDefinitionSpecificationFactory(
    private val repository: OperatonProcessDefinitionRepository,
    private var queryDialectHelper: QueryDialectHelper
) : AuthorizationSpecificationFactory<OperatonProcessDefinition> {

    override fun create(
        request: AuthorizationRequest<OperatonProcessDefinition>,
        permissions: List<Permission>
    ): AuthorizationSpecification<OperatonProcessDefinition> {
        return OperatonProcessDefinitionSpecification(
            request,
            permissions,
            repository,
            queryDialectHelper
    ...
    override fun canCreate(request: AuthorizationRequest<*>, permissions: List<Permission>): Boolean {
        return OperatonProcessDefinition::class.java == request.resourceType
    }
}