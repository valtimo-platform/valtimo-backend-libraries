/*
 * Copyright 2015-2020 Ritense BV, the Netherlands.
 *
 * Licensed under EUPL, Version 1.2 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * https://joinup.ec.europa.eu/collection/eupl/eupl-text-eupl-12
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" basis,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package com.ritense.valtimo.security.permission;

import com.ritense.valtimo.helper.DelegateTaskHelper;
import com.ritense.valtimo.service.BpmnModelService;
import com.ritense.valtimo.service.CamundaTaskService;
import org.camunda.bpm.engine.task.Task;
import org.springframework.security.core.Authentication;

public class PublicTaskAccessPermission implements Permission<String> {

    private final DelegateTaskHelper delegateTaskHelper;
    private final BpmnModelService bpmnModelService;
    private final CamundaTaskService camundaTaskService;

    public PublicTaskAccessPermission(
        final DelegateTaskHelper delegateTaskHelper,
        final BpmnModelService bpmnModelService,
        final CamundaTaskService camundaTaskService
    ) {
        this.delegateTaskHelper = delegateTaskHelper;
        this.bpmnModelService = bpmnModelService;
        this.camundaTaskService = camundaTaskService;
    }

    @Override
    public String name() {
        return "publicTaskAccess";
    }

    @Override
    public boolean isAllowed(final Authentication authentication, final String taskDefinitionId) {
        if (taskDefinitionId == null || taskDefinitionId.isEmpty()) {
            return false;
        }
        final Task taskEntity = camundaTaskService.findTaskById(taskDefinitionId);
        org.camunda.bpm.model.bpmn.instance.Task taskDefinition = bpmnModelService.getTask(taskEntity);
        return delegateTaskHelper.isTaskPublic(taskDefinition);
    }

}