...
public class OperatonOrderByHelper {

    private OperatonOrderByHelper() {
    }
    ...
    public static List<QueryOrderingProperty> sortToOrders(String operatonEntityName, Sort sort) {
        ...
        for (Sort.Order order : sort) {
            ...
            if (property.matches("^[0-9a-zA-Z$_]+$")) {
                try {
                    queryOrderingProperty = sortToOrder(operatonEntityName, property, direction);
    ...
    private static QueryOrderingProperty sortToOrder(
        String operatonEntityName,
        String queryPropertyName,
        String directionName
    ) {
        QueryProperty queryProperty;
        if ("HistoricProcessInstance".equals(operatonEntityName)) {
            queryProperty = findQueryPropertyForHistoricProcessInstance(queryPropertyName);
            // This can be expanded with more Operaton entity types. For each of these, another private static function should be added to this Class.
        } else {
            throw (new IllegalArgumentException("Invalid Operaton entity reference: " + operatonEntityName));
        }
