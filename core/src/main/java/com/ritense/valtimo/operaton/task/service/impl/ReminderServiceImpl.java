...
import static com.ritense.valtimo.operaton.repository.OperatonTaskSpecificationHelper.byCandidateGroups;
...
public class ReminderServiceImpl implements ReminderService {
    ...
    public ReminderServiceImpl(
            OperatonTaskService taskService, EmailNotificationSettingsService emailNotificationService,
    ...
    private List<RoleBasedTask> findRoleBasesTasksFor(String role) {
        final List<OperatonTask> tasks = tasksFor(role);
        if (tasks != null) {
            return tasks.stream()
                .map(task ->
                    new RoleBasedTask(
                        role,
                        new com.ritense.valtimo.operaton.task.domain.reminder.Task(
                            task.getId(),
                            task.getName(),
    ...
    private List<OperatonTask> tasksFor(String role) {
    ...
    private List<AssignedTask> findAssignedTasksFor(String assignee) {
        final List<OperatonTask> tasks = assignedTasks(assignee);
        if (tasks != null) {
            return tasks.stream()
                .map(task ->
                    new AssignedTask(
    ...
    private List<OperatonTask> assignedTasks(String assignee) {
