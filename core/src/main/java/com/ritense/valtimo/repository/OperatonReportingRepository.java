...
import com.ritense.valtimo.repository.operaton.dto.InstanceCount;
import com.ritense.valtimo.repository.operaton.dto.InstanceCountChart;
import com.ritense.valtimo.repository.operaton.dto.Serie;
...
public class OperatonReportingRepository {

    private final SqlSession session;
    private final OperatonRepositoryService repositoryService;

    public OperatonReportingRepository(SqlSession session, OperatonRepositoryService repositoryService) {
    ...
    public InstanceCountChart getProcessInstanceCounts(String processDefinitionKey) {
        ...
        var processDefinitionQuery = OperatonProcessDefinitionSpecificationHelper
            .byActive()
            .and(byLatestVersion());
        if (Optional.ofNullable(processDefinitionKey).isPresent()) {
            processDefinitionQuery.and(byKey(processDefinitionKey));
        }
        List<OperatonProcessDefinition> deploydDefinitions = AuthorizationContext
