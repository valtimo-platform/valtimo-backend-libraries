...
class IkoDataAggregateSpecificationFactory(
    private val ikoDataAggregateRepository: IkoDataAggregateRepository,
    private var queryDialectHelper: QueryDialectHelper,
) : AuthorizationSpecificationFactory<IkoDataAggregate> {

    override fun create(
        request: AuthorizationRequest<IkoDataAggregate>,
        permissions: List<Permission>
    ): AuthorizationSpecification<IkoDataAggregate> {
        return IkoDataAggregateSpecification(
            request,
            permissions,
    ...
    override fun canCreate(context: AuthorizationRequest<*>, permissions: List<Permission>): Boolean {
        return IkoDataAggregate::class.java == context.resourceType
    }
}