...
@Transactional
internal class IkoDataAggregateResourceTest {

    private lateinit var mockMvc: MockMvc
    private lateinit var resource: IkoDataAggregateResource
    private lateinit var service: IkoDataAggregateService

    @BeforeEach
    fun init() {
        service = mock()
        resource = IkoDataAggregateResource(service)
    ...
    fun `should get iko dataAggregates`() {
        ...
        mockMvc.perform(get("/api/v1/iko-data-aggregate?key={key}&title={title}", "klant", "Klant"))
            .andDo(print())
