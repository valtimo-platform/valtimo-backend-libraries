...
open class SearchFieldV2DeploymentService(
    private val objectMapper: ObjectMapper,
    private val repo: SearchFieldV2Repository,
    private val changelogService: ChangelogService,
    private val clearTables: Boolean
) : ChangesetDeployer {
    override fun getPath() = "classpath*:**/*.task-search-field.json"

    override fun before() {
        if (clearTables) {
            repo.deleteAllByOwnerType("TaskListSearchColumns")
    ...
    override fun getChangelogDetails(filename: String, content: String): List<ChangesetDetails> {
        ...
        return listOf(
            ChangesetDetails(
                ...
                valueToChecksum = changeset.searchFields,
                key = KEY,
                deploy = { deploy(changeset.searchFields) }
            )
        )
    }

    private fun deploy(searchFields: List<SearchFieldV2>) {
        runWithoutAuthorization {
            repo.saveAll(searchFields)
        }
    }
